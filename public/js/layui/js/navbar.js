		...
		for(var i = 0; i < data.length; i++) {
			...
			if(data[i].children !== undefined && data[i].children.length > 0) {
				...
				for(var j = 0; j < data[i].children.length; j++) {
					...
					if(data[i].children[j].number !== undefined){
						ulHtml += '<div style="float:right;width:24px;height:19px;background-color:'+data[i].children[j].color+';margin:10px 0;border-radius: 3px;"><p new-id='+data[i].children[j].new1+' style="color:#fff;margin-top: -10px;text-align: center;font-weight: bold">' + data[i].children[j].number + '</p></div>';
					}
					ulHtml += '</a>';
					ulHtml += '</dd>';
				}
